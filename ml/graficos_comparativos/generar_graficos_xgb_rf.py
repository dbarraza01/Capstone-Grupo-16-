"""
Gráficos Comparativos: XGBoost Final vs Random Forest Final
==============================================================
...
OUT  = Path(__file__).resolve().parent / "xgb_vs_rf"
...
for ax, df, color, title in [
    (axes[0], rf,  COLOR_RF,  "Random Forest Final"),
...
fig.suptitle("Gráfico 1: LOS Real vs. Predicho — Random Forest vs. XGBoost\n(Mismo dataset Escenario B, misma partición)", fontweight="bold")
plt.tight_layout()
...
ax.hist(rf["error"],  bins=60, alpha=0.55, color=COLOR_RF,  label="Random Forest Final", density=True)
ax.hist(xgb["error"], bins=60, alpha=0.55, color=COLOR_XGB, label="XGBoost Final",    density=True)
ax.axvline(0, color="black", lw=1.5, linestyle="--")
ax.axvline(rf["error"].mean(),  color=COLOR_RF,  lw=2, linestyle=":", label=f"Bias RF = {rf['error'].mean():.2f} d")
...
b1 = ax.bar(x - w/2, mae_rf,  w, label="Random Forest Final", color=COLOR_RF,  alpha=0.85)
...
ax.set_title("Gráfico 3: MAE por Tramo de LOS — Random Forest vs. XGBoost\n(Mismo dataset, misma partición)", fontweight="bold")
ax.legend()
plt.tight_layout()
...
rf_plos_real  = (rf["los_real"] >= 27).astype(int)
rf_plos_pred  = (rf["los_pred"] >= 27).astype(int)
xgb_plos_real = (xgb["los_real"] >= 27).astype(int)
xgb_plos_pred = (xgb["los_pred"] >= 27).astype(int)

from sklearn.metrics import precision_score, recall_score, f1_score

rf_prec  = precision_score(rf_plos_real, rf_plos_pred, zero_division=0)
rf_rec   = recall_score(rf_plos_real, rf_plos_pred, zero_division=0)
rf_f1    = f1_score(rf_plos_real, rf_plos_pred, zero_division=0)
...
b1 = ax.bar(x - w/2, rf_vals,  w, label="Random Forest Final", color=COLOR_RF,  alpha=0.85)
...
ax.set_title("Gráfico 5: Métricas Clínicas PLOS (≥ 27 días)\nRandom Forest vs. XGBoost (Mismo dataset)", fontweight="bold")
...
ax.plot(TRAMOS, pct_sub_rf,  "o-", color=COLOR_RF,  lw=2, markersize=8, label="Random Forest Final")
...
for i, (y_rf, y_xgb) in enumerate(zip(pct_sub_rf, pct_sub_xgb)):
    ax.annotate(f"{y_rf:.0f}%",  (TRAMOS[i], y_rf),  textcoords="offset points", xytext=(-18, 6),  fontsize=9, color=COLOR_RF)
...
cm_rf  = confusion_matrix(rf_plos_real, rf_plos_pred, labels=[0, 1])
cm_xgb = confusion_matrix(xgb_plos_real, xgb_plos_pred, labels=[0, 1])

matrices = {
    "Random Forest Final": cm_rf,
    "XGBoost Final":       cm_xgb,
}

fig, axes = plt.subplots(1, 2, figsize=(12, 5))

for ax, (title, cm), color in zip(axes, matrices.items(), [COLOR_RF, COLOR_XGB]):
    total = cm.sum()
    im = ax.imshow(cm, cmap="Blues" if color == COLOR_XGB else "Greens", aspect="auto")
...
# ============================================================================
rf_mae   = rf["abs_error"].mean()
rf_rmse  = np.sqrt((rf["error"]**2).mean())
rf_medae = rf["abs_error"].median()
rf_bias  = rf["error"].mean()
...
resumen = pd.DataFrame({
    ...
    "Random Forest": [round(rf_mae,3), round(rf_rmse,3), round(rf_medae,3),
                      round(rf_prec*100,1), round(rf_rec*100,1), round(rf_f1*100,1), round(rf_bias,2)],
...
for _, row in resumen.iterrows():
    m = row["Métrica"]
    rf_v, xgb_v = row["Random Forest"], row["XGBoost"]
    if m in ["MAE (días)", "RMSE (días)", "MedAE (días)"]:
        better.append("XGBoost" if xgb_v < rf_v else "Random Forest")
    elif m == "Recall PLOS":
        better.append("Random Forest" if rf_v > xgb_v else "XGBoost")
    elif m == "Sesgo (días)":
        better.append("XGBoost" if abs(xgb_v) < abs(rf_v) else "Random Forest")
    else:
        better.append("XGBoost" if xgb_v > rf_v else "Random Forest")
...
for (row, col), cell in table.get_celld().items():
    if row == 0:
        ...
        cell.set_facecolor(COLOR_XGB if winner == "XGBoost" else COLOR_RF)
        cell.set_text_props(color="white", fontweight="bold")
    elif row % 2 == 0:
...
print("\n✅ Todos los gráficos generados en:", OUT / "xgb_vs_rf")
