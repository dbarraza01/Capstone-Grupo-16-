...
# Definición del modelo XGBoost inicial 
# las justificaciones de los hiperparametros recordar colocarlas en el informe
xgb_model = XGBRegressor(
    n_estimators=300, # número de árboles de decisión que formarán el ensamble.
    max_depth=6, # profundidad máxima de cada árbol.
    learning_rate=0.05, # tasa de aprendizaje, controla el impacto de cada nuevo árbol. Un valor más bajo requiere más árboles pero generaliza mejor.
    subsample=0.8, # fracción de observaciones usadas para ajustar cada árbol. Ayuda a prevenir sobreajuste.
    colsample_bytree=0.8, # fracción de características seleccionadas al azar para cada árbol.
    random_state=42, # para reproducibilidad.
    n_jobs=-1, # utiliza todos los hilos disponibles.
    objective='reg:squarederror' # función objetivo a optimizar.
)
...
y_pred_log = xgb_model.predict(X_test) # predicción en log(LOS+1)
...
cm_df.to_csv(REPORT_DIR / "matriz_confusion_plos_xgb_v1.csv", sep=";", index=False)
...
df_pred.to_csv(
    REPORT_DIR / "predicciones_xgboost_v1.csv",
    sep=";",
    index=False
)
...
metricas_globales.to_csv(
    REPORT_DIR / "metricas_xgboost_v1.csv",
    sep=";",
    index=False
)

metricas_tramo.to_csv(
    REPORT_DIR / "metricas_por_tramo_xgboost_v1.csv",
    sep=";",
    index=False
)

joblib.dump(xgb_model, OUT_DIR / "xgboost_v1.pkl") # guardamos el modelo entrenado, permite reutilizarlo en el futuro sin tener que entrenar de nuevo


print("\nArchivos guardados:")
print(REPORT_DIR / "predicciones_xgboost_v1.csv")
print(REPORT_DIR / "metricas_xgboost_v1.csv")
print(REPORT_DIR / "metricas_por_tramo_xgboost_v1.csv")
print(REPORT_DIR / "matriz_confusion_plos_xgb_v1.csv")
print(OUT_DIR / "xgboost_v1.pkl")
