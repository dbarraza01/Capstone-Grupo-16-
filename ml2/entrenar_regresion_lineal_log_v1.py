...
# ============================================================================
# Modelo: Regresión lineal sobre log(1 + LOS)
# ============================================================================

modelo_lineal_log = LinearRegression()

y_train_log = np.log1p(y_train)

print("\nEntrenando regresión lineal sobre log(1 + LOS)...")
modelo_lineal_log.fit(X_train, y_train_log)

y_pred_log = modelo_lineal_log.predict(X_test)

# Volver desde escala logarítmica a días
y_pred_lineal_log = np.expm1(y_pred_log)
y_pred_lineal_log = np.clip(y_pred_lineal_log, 0, None)

metricas_log, metricas_tramo_log, pred_log = evaluar_modelo(
    nombre_modelo="regresion_lineal_log1p_los",
    y_test=y_test,
    y_pred=y_pred_lineal_log,
    ids_test=ids_test,
    report_dir=REPORT_DIR
)

joblib.dump(modelo_lineal_log, OUT_DIR / "regresion_lineal_log1p_los.pkl")

print("\nArchivos guardados correctamente (Modelo Log).")
