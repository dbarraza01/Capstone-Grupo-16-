...
UMBRAL_PLOS = 14
TRAMOS_BINS = [-1, 2, 6, 13, np.inf]
TRAMOS_LABELS = ["0-2", "3-6", "7-13", "14+ (PLOS)"]
...
def calcular_metricas_plos(
    y_true: Iterable[float],
    y_pred: Iterable[float],
    umbral: int = UMBRAL_PLOS,
) -> dict[str, float]:
    """Metricas de deteccion PLOS usando LOS real/predicho >= umbral."""
    y_true_arr, y_pred_arr = _validar_vectores(y_true, y_pred)
    plos_real = y_true_arr >= umbral
    plos_pred = y_pred_arr >= umbral
    ...
    return {
        "umbral_plos": int(umbral),
        "n_plos_real": int(np.sum(plos_real)),
        "n_plos_pred": int(np.sum(plos_pred)),
        "precision_plos_14": float(precision),
        "recall_plos_14": float(recall),
        "f1_plos_14": float(f1),
        "accuracy_plos_14": float(accuracy),
        "tn_plos_14": tn,
        "fp_plos_14": fp,
        "fn_plos_14": fn,
        "tp_plos_14": tp,
...
def calcular_metricas_globales(
    ...
    metricas.update(calcular_metricas_plos(y_true_arr, y_pred_arr))
    return metricas
    ...
    for tramo in TRAMOS_LABELS:
        ...
        if not mask.any():
            filas.append({
                ...
                "precision_plos_14": np.nan,
                "recall_plos_14": np.nan,
                "f1_plos_14": np.nan,
                "tp_plos_14": np.nan,
                "fp_plos_14": np.nan,
                "fn_plos_14": np.nan,
...
def formatear_metricas_markdown(metricas: dict[str, float]) -> str:
    ...
    for clave in orden:
        ...
        elif clave.startswith("pct_") or clave == "pup" or clave in {
            "precision_plos_14",
            "recall_plos_14",
            "f1_plos_14",
            "accuracy_plos_14",
