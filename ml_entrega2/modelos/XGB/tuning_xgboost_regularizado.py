"""
tuning_xgboost_regularizado.py
==============================
Búsqueda regularizada de hiperparámetros para XGBoost.
...
# =============================================================================
# La búsqueda anterior permitió modelos XGB muy potentes:
# muchos árboles, profundidad alta y train MAE muy bajo.
# Esta versión reduce profundidad, baja learning rate y aumenta regularización.

PARAM_DISTRIBUTIONS = {
    "regressor__n_estimators": randint(300, 900),
    "regressor__max_depth": randint(3, 8),              # 3 a 7
    "regressor__learning_rate": uniform(0.01, 0.09),    # 0.01 a 0.10
    "regressor__subsample": uniform(0.6, 0.3),          # 0.6 a 0.9
    "regressor__colsample_bytree": uniform(0.4, 0.4),   # 0.4 a 0.8
    "regressor__min_child_weight": randint(3, 15),
    "regressor__gamma": uniform(1, 6),                  # 1 a 7
    "regressor__reg_alpha": uniform(0, 5),
    "regressor__reg_lambda": uniform(1, 8),
}
...
def run_tuning(X, y, ids, scenario_name, target_mode):
    print("\n" + "=" * 80)
    print(f"XGBoost regularizado | {scenario_name} | target={target_mode}")
    ...
    pred_path = OUT_DIR / f"predicciones_xgb_regularizado_{scenario_name}_{target_mode}.csv"
    tramo_path = OUT_DIR / f"metricas_tramos_xgb_regularizado_{scenario_name}_{target_mode}.csv"
...
def main():
    print("=" * 80)
    print("XGBoost regularizado — tuning con holdout test")
    print("=" * 80)
    ...
    for scenario_name, path in DATASETS.items():
        ...
        for target_mode in TARGET_MODES:
            ...
            summary_rows.append({
                "modelo": "XGBRegressor_regularizado",
    ...
    json_path = OUT_DIR / "mejores_hiperparametros_xgboost_regularizado.json"
    csv_path = OUT_DIR / "resumen_tuning_xgboost_regularizado.csv"
